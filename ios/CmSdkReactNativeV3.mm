...
- (instancetype)init {
    if (self = [super init]) {
        _implementation = [[CmSdkReactNativeV3Impl alloc] initWithEventEmitter:self];
    }
    return self;
}

- (NSArray<NSString *> *)supportedEvents {
    return @[@"didReceiveConsent", @"didShowConsentLayer", @"didCloseConsentLayer", 
             @"didReceiveError", @"onClickLink", @"didChangeATTStatus"];
}

- (std::shared_ptr<facebook::react::TurboModule>)getTurboModule:
    (const facebook::react::ObjCTurboModule::InitParams &)params {
    return std::make_shared<facebook::react::NativeCmSdkReactNativeV3SpecJSI>(params);
}
...
- (void)setWebViewConfig:(JS::NativeCmSdkReactNativeV3::WebViewConfig &)config
    ...
    if (config.position()) configDict[@"position"] = config.position();
...
- (void)setATTStatus:(double)status
             resolve:(RCTPromiseResolveBlock)resolve
              reject:(RCTPromiseRejectBlock)reject {
    [_implementation setATTStatus:(NSInteger)status resolve:resolve reject:reject];
}
...
- (void)acceptPurposes:(NSArray *)purposes
         updatePurpose:(BOOL)updatePurpose
               resolve:(RCTPromiseResolveBlock)resolve
                reject:(RCTPromiseRejectBlock)reject {
    [_implementation acceptPurposes:purposes updatePurpose:updatePurpose resolve:resolve reject:reject];
}
...
- (void)rejectPurposes:(NSArray *)purposes
          updateVendor:(BOOL)updateVendor
               resolve:(RCTPromiseResolveBlock)resolve
                reject:(RCTPromiseRejectBlock)reject {
    [_implementation rejectPurposes:purposes updateVendor:updateVendor resolve:resolve reject:reject];
}
